...
@Component({
    tag: 'agc-adjusted-scrotal-circumference-results'
})
export class AgcAdjustedScrotalCircumferenceResults {
    @Prop() socket: string = ""
    @State() data: any
                ...
                <div style={{display: this.ready ? 'block' : 'none'}}>
                    {this.data && (<ul class="agc-results">
                            <li>
                                ...
                                <span class="agc-results__value">{this.data['adjustedYearlingScrotalCircumference']}</span>
                                <sub>{this.data['units']['length']}</sub>
                            </li>
                            <li>
